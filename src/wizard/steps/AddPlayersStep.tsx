...
export const AddPlayersStep: React.FC = () => {
  ...
  const areAllPlayerNamesFilled = players.ids.every(
    (id) => players.names[id].trim().length > 0
  );
  ...
  const handleAddPlayer = () => {
    const firstFreeColor = AVAILABLE_PLAYER_COLORS.find(
      (color) => !isColorTaken(color)
    );
  ...
  return (
    ...
    >
      <Box
        ...
        gridTemplateRows={`repeat(${players.ids.length}, 56px) ${
          canAddPlayer ? 'auto' : ''
        } auto`}
        gridColumnGap={16}
        gridRowGap={16}
      >
        {players.ids.map((id) => (
          <React.Fragment key={id}>
            <TextField
              ...
              variant="outlined"
              label="Name"
              value={players.names[id]}
              onChange={(event) => handleNameChange(event, id)}
            />
            <FormControl>
              <InputLabel id={`color-select-${id}`} variant="outlined">
                Color
              </InputLabel>
              <Select
                color="primary"
                ...
                onChange={(event) =>
                  handleColorChange(event.target.value as string, id)
                }
              >
                {AVAILABLE_PLAYER_COLORS.map((color) => (
                  <MenuItem
                    key={color}
                    value={color}
                    disabled={isColorDisabledForPlayer(id, color)}
                  >
        ...
        <Box
          gridColumn={`1 / ${canRemovePlayer ? 4 : 3}`}
          display="grid"
        ...
        >
          ...
          <Button
            color="primary"
            variant="contained"
            disabled={!areAllPlayerNamesFilled}
            onClick={handleNext}
          >
            NEXT
          </Button>
