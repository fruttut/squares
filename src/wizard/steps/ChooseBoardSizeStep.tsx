  ...
  return (
    <Box display="grid" flexGrow={1} gridTemplateColumns="repeat(3, 1fr)">
      {AVAILABLE_BOARD_DIMENSIONS.map(availableDim => (
        <Box key={availableDim} display="flex" justifyContent="center" alignItems="center">
          <BoardCard dim={availableDim} selected={dim === availableDim} onClick={handleBoardCardClick} />
        </Box>
      ))}
    </Box>
  );
