...
import {excludeAllFromArray, excludeAllFromObject, excludeItemFromArray} from "../utils/exclude";

const isCellPainted = (state: IArenaState, id: string) => state.cells.groups[id] != null;

const isCellPaintedBy = (state: IArenaState, cellId: string, playerId: string) => {
...
const addStandaloneCell = (state: IArenaState, cellId: string, playerId: string, groupId: string) => ({
  ...state,
  cells: {
    ...state.cells,
    groups: {
...
const getPlayerNeighbourCells = (state: IArenaState, cellId: string, playerId: string) => {
  return getNeighbourCells(state, cellId).filter((neighbourId) => isCellPaintedBy(state, neighbourId, playerId));
};
...
export const arenaReducer = (state: IArenaState, action: ArenaAction) => {
  switch (action.type) {
    case EArenaActionTypes.PAINT_CELL: {
      ...
      const neighbourCellIds = getPlayerNeighbourCells(state, action.cellId, action.playerId);
      if (neighbourCellIds.length === 0) {
        return addStandaloneCell(state, action.cellId, action.playerId, action.groupId);
      }
      ...
      const otherGroupIds = unique(excludeItemFromArray(otherNeighbours.map(id => state.cells.groups[id]!), targetGroupId));
      const cellsToMove = otherGroupIds.flatMap(id => state.groups.cells[id]);
      return {
        ...state,
        cells: {
          ...state.cells,
          groups: populateObject(state.cells.groups, [...cellsToMove, action.cellId], targetGroupId),
        },
        groups: {
          ...state.groups,
          ...
          cells: {
            ...
            [targetGroupId]: [...state.groups.cells[targetGroupId], ...cellsToMove, action.cellId],
          },
        },
        score: {
          ...state.score,
          [action.playerId]: Math.max(state.score[action.playerId], targetGroupCount + cellsToMove.length + 1),
        },
      };
    }
    case EArenaActionTypes.PASS_MOVE:
      return {
        ...state,
        currentTurnIndex: state.currentTurnIndex === state.turns.length - 1 ? 0 : state.currentTurnIndex + 1,
